...
led::color_t pixelcolor[NUM_LEDS];
uint8_t brightness[NUM_LEDS];

led::led(){
    for(uint32_t i=0;i<NUM_LEDS;i++){
        brightness[i]=255;
        pixelcolor[i].value=0x00FFFFFF;
    }
...
void led::setColor(uint32_t led, color_t color ){
    if(led<NUM_LEDS){
        pixelcolor[led]=color;
        uint16_t r,g,b;
        r=color.rgbw.r*(255-(255-brightness[led]));
        g=color.rgbw.g*(255-(255-brightness[led]));
        b=color.rgbw.b*(255-(255-brightness[led]));
        r=r/255;
        g=g/255;
        b=b/255;
        ws2812fx.setPixelColor(led,color.rgbw.r,color.rgbw.g,color.rgbw.b);
    }
}
...
void led::init(void){
    if( (NUM_LEDS>0) && (LED_PIN>=0)){
        ...
        ws2812fx.setColor(0,pixelcolor[0].value);
        ws2812fx.setColor(1,pixelcolor[1].value);
        ws2812fx.setColor(2,pixelcolor[2].value);
